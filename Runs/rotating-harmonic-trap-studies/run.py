#! /bin/env python
#@+leo-ver=4-thin
#@+node:gcross.20100106124611.2054:@thin run.py
#@@first
#@<< Imports >>
#@+node:gcross.20100106124611.2056:<< Imports >>
import gc
...
#@+others
#@+node:gcross.20100106124611.2057:class RotationEffectivePotential
class RotationEffectivePotential(Physics):
    #@    @+others
    #@+node:gcross.20100106124611.2058:hooks
    hooks = ["effective_potentials"]
    #@-node:gcross.20100106124611.2058:hooks
    #@+node:gcross.20100106124611.2059:__init__
    ...
    #@-node:gcross.20100106124611.2059:__init__
    #@+node:gcross.20100106124611.2060:accumulate_potential
    ...
    #@-others
#@-node:gcross.20100106124611.2057:class RotationEffectivePotential
#@+node:gcross.20100106124611.2061:class HarmonicOscillator
class HarmonicOscillator(Physics):
  #@  @+others
  #@+node:gcross.20100106124611.2062:hooks
  hooks = ["physical_potentials","trial_functions"]
  #@-node:gcross.20100106124611.2062:hooks
  #@+node:gcross.20100106124611.2063:__init__
  def __init__(self,system):
      Physics.__init__(self,system)
      try:
  ...
  #@-node:gcross.20100106124611.2063:__init__
  #@+node:gcross.20100106124611.2064:accumulate_potential
  def accumulate_potential(self,x,xij2,U,gradU2):
  ...
  #@-node:gcross.20100106124611.2064:accumulate_potential
  #@+node:gcross.20100106124611.2065:compute_trial_weight
  ...
  #@-node:gcross.20100106124611.2065:compute_trial_weight
  #@+node:gcross.20100106124611.2066:accumulate_trial_derivatives
...
#@+node:gcross.20100113111641.2593:<< System Configuration >>
configuration = {
    # System parameters
...
#@+node:gcross.20100106124611.2067:<< Histogram Configuration >>
...
#@+node:gcross.20100106124611.2068:<< System properties message >>
...
for frame_angular_velocity in [-1]:
    for number_of_particles in [10]:
        for number_of_rotating_particles in xrange(2*number_of_particles+1):
            ...
            #@+node:gcross.20100106124611.2069:<< Run simulation for given parameters >>
            my_directory = "{output_root_directory}/{frame_angular_velocity}/{number_of_particles}".format(**vars()) 
            if (my_rank == 0):
                print
            ...
            #@<< Initialize physics >>
            #@+node:gcross.20100106124611.2070:<< Initialize physics >>
            for physics in [
                HarmonicOscillator,
            ...
            #@+node:gcross.20100106124611.2071:<< Initialize observables >>
